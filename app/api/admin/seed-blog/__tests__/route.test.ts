// app/api/admin/seed-blog/__tests__/route.test.ts
...
// seed-blog uses supabase.from().upsert().select() — mock the full chain
vi.mock('@supabase/supabase-js', () => ({
  createClient: vi.fn().mockReturnValue({
    from: vi.fn().mockReturnValue({
      upsert: vi.fn().mockReturnValue({
        select: vi.fn().mockResolvedValue({ data: [{ id: 'id-1', slug: 'test' }], error: null }),
      }),
...
describe('POST /api/admin/seed-blog', () => {
  it('returns 403 immediately when requireAdminAccess denies access', async () => {
    ...
    const request = new Request('http://localhost/api/admin/seed-blog', { method: 'POST' })
  ...
  it('returns 200 with success:true and seeded count on happy path', async () => {
