// app/api/admin/migrate-teams/__tests__/route.test.ts
...
describe('POST /api/admin/migrate-teams', () => {
  it('returns 403 immediately when requireAdminAccess denies access', async () => {
    ...
    const request = new Request('http://localhost/api/admin/migrate-teams', { method: 'POST' })
  ...
  it('returns 200 with results array on happy path', async () => {
    ...
    // Route returns 200 when all ok, 207 when some fail — we stub fetch as ok:true so all ok
