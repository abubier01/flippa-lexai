...
--
-- Creates:
--   * public.profiles.is_platform_admin (boolean)
...
-- ---------------------------------------------------------------------------
-- (0) Admin flag on public.profiles + bootstrap admin row.
--
-- Repo uses public.profiles (1:1 FK to auth.users(id), auto-created by the
-- on_auth_user_created trigger). The spec wrote public.users; correcting to
-- profiles is a docs/spec drift, not a design change.
-- ---------------------------------------------------------------------------
...
CREATE TABLE persona_versions (
  ...
  notes           text,
  created_at      timestamptz NOT NULL DEFAULT now(),
  -- FK to auth.users(id) directly (matches contracts.user_id pattern in 001).
  -- profiles is 1:1 with auth.users so either would work; auth.users is the
...
CREATE POLICY personas_admin_all ON personas
  USING (EXISTS (SELECT 1 FROM public.profiles u WHERE u.id = auth.uid() AND u.is_platform_admin = true))
  WITH CHECK (EXISTS (SELECT 1 FROM public.profiles u WHERE u.id = auth.uid() AND u.is_platform_admin = true));
